...
<div class="row collapse p-1" id="filtro{{ ucfirst($nombre_grupo) }}">
    <div class="container">
        @foreach ($grupo as $elemento)
            <div class="row">
                @php
                    ...
                    } elseif (isset($areaGestion) && $areaGestion) {
                        if ($areaGestion == $elemento->id) {
                            $chequeado = 'checked';
                            $restriccion = '';
                            $clase_etiqueta = '';
                        } else {
        ...
        <div class="row">
            <button type="button" id="reset-filter-areas" class="grupo-menu-izquierda formacion" style="border: none; margin: auto; margin-top: 15px">
                Restablecer filtro
            </button>
        </div>
    </div>
