            ...
            <tr>
                ...
                <div class="certificado">
                    ...
                    <div class="contenido">
                        ...
                        @php
                            if($duracion_sesion != null && $duracion_sesion != ''){
                                $existeDuracion = true;
                            }else{
                                $existeDuracion = false;
                            }

                            if($creditos!= null && $creditos != 0.00){
                                $existeCreditos = true;
                            }else{
                                $existeCreditos = false;
                            }

                        @endphp
                        <p class="no-break">
                            Con NIF <span class="resaltado">{{ $dni }}</span> ha participado  como <span class="resaltado">{{ $tipo_colaboracion }}</span> en el <span class="resaltado">{{ $nombre_curso }}</span> que tuvo lugar <span class="resaltado">{{ $fechas_sesion }}</span>@if(!$existeDuracion && !$existeCreditos). @endif
                            @if($existeDuracion) con una duración de 
                            <span class="resaltado">{{ $duracion_sesion }}</span> hora(s) lectiva(s)@if(!$existeCreditos). @endif
                            @endif @if($existeCreditos) y con <span class="resaltado">{{ $creditos }} crédito(s).</span>@endif
                        </p>
