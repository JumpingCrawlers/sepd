...
class LanzamientoPagoMetodoRecibo2022Fix extends Migration
{
    /**
     * 3ways - Euro Fuenmayor
     * Migración requerida fix lanzamiento de pago 2022 - Forma de pago: Recibo domiciliado
     * Run the migrations.
     *
     * @return void
     */
...

    {
        ...
        $usuarios_que_pagan_via_recibo_con_estado_pago_2022_ya_creado = DB::table('usuarios_pagos')
            ...
            ->where('usuarios_socios.socios_pagos_metodos_id', 14)
            ->whereNull('usuarios_pagos.deleted_at')
            ->whereNull('usuarios_pagos_socios.deleted_at')
            ->whereNull('usuarios_socios.deleted_at')
            ->get();

        $usuarios_pendientes_de_pagar_via_recibo = DB::table('usuarios_socios')
            ->join('socios_tipos', 'socios_tipos.id', '=', 'usuarios_socios.tipo_socio')
            ->where('usuarios_socios.socios_pagos_metodos_id', 14)
            ->where('socios_tipos.cuota', '>', 0)
            ->whereNull('usuarios_socios.deleted_at')
            ->whereNotIn('usuarios_socios.usuario_id', array_column($usuarios_que_pagan_via_recibo_con_estado_pago_2022_ya_creado->toArray(), 'usuario_id'))
            ->get();

        foreach ($usuarios_pendientes_de_pagar_via_recibo as $row) {
        ...
        \Illuminate\Support\Facades\Log::debug('$usuarios_pendientes_de_pagar_via_recibo fix 2022-10-02 count: '.print_r($usuarios_pendientes_de_pagar_via_recibo->count(),true));
    }
